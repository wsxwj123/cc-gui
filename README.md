...
<details>
<summary><b>v0.2.399</b>(2026-09-21)· 修复 4 条</summary>
...
<details><summary>发消息不再等回滚点清理</summary>

以前每发一条消息,都要先把这个会话所有回滚点逐个精确算一遍占用才肯放行,历史越长等得越久(实测 1 条快照的会话 60 毫秒、25 条的 246 毫秒)。现在响应里只做摘除记录这种毫秒级的事,真正费时的腾空间挪到后台并节流(默认 30 秒,可用 CGUI_CHECKPOINT_REPACK_DEBOUNCE_MS 调整);同一会话改完后是 37 毫秒对 44 毫秒。列表条数、天数上限、总占用上限的表现都和以前一样,主动删除仍然立刻释放空间。

</details>

<details><summary>启动清扫不会再和拍快照打架</summary>

程序启动后的回滚点清扫,正好扫到你在用的那个会话时,可能让新快照创建失败、列表和实际对不上,极端情况下损坏该会话的回滚点仓库。现在拍快照、回滚、删除、清扫、后台回收共用一把会话锁,拍快照会等锁而不是失败;刚写入的数据另有一道 2 分钟保护不被误删。

</details>

<details><summary>开头带 BOM 的配置文件不再被当成损坏</summary>

Windows 记事本和 PowerShell 保存文件时默认会在开头写 3 个不可见字节(BOM),以前这会让完全正确的 provider、模型、生图配置被判定为损坏——读成空、锁住写入,提示还建议你删掉它。现在解析前会剥掉 BOM 正常读取,写回也不带 BOM;真正损坏时的提示改为提醒你文件开头可能有 BOM 或不可见字符,请用能显示 BOM 的编辑器检查。
...
<details><summary>「过程块自动折叠」做成设置项,默认不折叠</summary>

设置面板第一页新增开关。关闭(默认)时,已结束回合的思考与工具调用保持展开、留在各段正文之间;开启则沿用此前的行为(回合结束自动收成一行摘要)。正在生成的回合始终展开;手动点某一轮的摘要行只影响那一轮。

</details>

<details><summary>回滚点启动清扫</summary>

服务启动 60 秒后在后台逐个检查所有会话的回滚点,超过条数 / 天数 / 总占用上限的会话按既有规则回收(从最旧开始、保底最新一条),不再需要该会话"下次拍快照"才触发;10 分钟内有活动的会话跳过;没超限的会话只做便宜检查;每次启动只扫一遍,日志留一行汇总。

</details>

- 内置 provider 预设新增 DMXAPI、云雾两家第三方中转(需自备 key)。

**修复**

- 用量页「价格与来源」版块常态只显示按钮与三个折叠项;原先直接铺开的四行说明(范围、抓取时间、各状态计数、原币种条数)收进对应折叠项里,「刷新全部」入口保留在折叠内;按钮不可用时仍在折叠外说明原因。

- 订阅额度区把"命令行工具没登录"与"账户没有订阅额度"分开:未登录时明说未登录并给出 `claude auth login`,不再误报"该账户没有官方订阅额度"。
...
<details><summary>点「停止」和按 Esc 立刻有反应</summary>

以前在模型输出密集时，界面每个字都要把整篇内容重新排版一遍，主线程被占满，你按下去的那一下最多要排队 3 秒才被受理、之后处理又要 8 秒（最坏实测 6–11 秒才看到反应）。现在改为每帧最多重排一次，并且已经排好的段落不再重复排版。实测：长会话点停止从 6474 毫秒降到 **51 毫秒**，主线程最长卡顿从 8.3 秒降到 **66 毫秒**。

</details>

<details><summary>点停止后不再长时间显示「运行中」</summary>

现在按下的瞬间按钮就进入「停止中」（不等服务器回话），真正结束才恢复。

</details>

- 会话正在输出时，滚动、点击、切换等操作不再被拖住。

</details>

<details>
<summary><b>v0.2.391</b>(2026-09-15)· 修复 1 条</summary>

**修复**

<details><summary>切走会话再切回来,刚发的那条消息不再消失</summary>

回合进行中切到别的会话、再切回来时,刚发出的消息以前会消失十几秒(等会话进程开始输出后才回来)——因为切走会清掉本地那条消息,而此时它还没写进会话记录。现在切回后本地那份会保留到会话记录里出现为止,并且不会重复显示、也不会串到别的会话。会话进程启动较慢时(装的扩展较多)最容易遇到。
...
<details><summary>电脑操控输入的文字不再出现在进程列表里</summary>

往窗口里输入的文字(可能是密码)和窗口标题,以前作为命令行参数传给执行进程,本机其他程序用 `ps` 就能看到明文。现在改由内部通道传递。

</details>

<details><summary>以 `-` 开头的文字可以正常输入</summary>

输入 `--dry-run` 这类文字,或目标窗口标题以 `-` 开头(如终端的 `-zsh`)时,以前会直接报错。

</details>

<details><summary>读不回内容的输入框,长文字完整送达</summary>

以前改走备用投递时,整段文字放进一个键盘事件,只能打出开头一小段;现在逐字投递。备用投递失败或超时时如实报错,不再回"已投递"。

</details>

<details><summary>电脑操控超时的处理</summary>

超时现在报"超时"而不是"投递失败";结束执行进程前先让它松开按住的修饰键和鼠标左键。

</details>

<details><summary>不再向模型交出未授权应用的信息</summary>

窗口列表不再包含未授权前台应用(如密码管理器)的名称和进程号;未授权全屏时,光标位置只给出已授权窗口内的坐标。

</details>

<details><summary>截图目录不再无限增长</summary>

已退出会话留下、超过 24 小时的截图会被清理。

</details>

<details><summary>终端打开超时后不再遗留后台进程</summary>

超时后才启动成功的终端会被关闭,不占用终端名额。

</details>

<details><summary>价格缓存与用量说明不会因中途退出被清空</summary>

写入改为整份替换。

</details>

<details><summary>价格刷新失败后可以立即重试</summary>

以前失败后 24 小时内再点「刷新价格」不会重新抓取。

</details>

<details><summary>同一会话的两个历史操作同时提交时,只有一个生效</summary>

另一个提示冲突,不再两边都显示成功。

</details>

<details><summary>一天内发送满 512 条后仍可继续发送</summary>

以前之后的发送都会被拒绝,直到最早的记录满 24 小时。

</details>

<details><summary>会话进程启动即崩溃时显示真实原因</summary>

以前误报为"15 秒内未完成初始化,请勿据此重发"。
...
<details>
<summary><b>v0.2.388</b>(2026-09-14)· 修复 3 条</summary>

**修复**

<details><summary>切换 provider 后，用量页的「范围」行跟着变了</summary>

以前它只在打开面板那一刻读一次——你在顶栏切了 provider，卡片还显示着旧的那家（真正刷新的对象是对的，是**旁边的说明在骗人**）。现在跟着变。

</details>

<details><summary>deepseek 发图不再被静默丢弃</summary>

官方从 DeepSeek-V4.1-Flash 起把看图能力并进了 Flash 系，**模型名里不再带 "vision"**，而界面判断"这个模型能不能收图"用的是一张按名字模式匹配的表 —— 于是 `deepseek-flash` 被误判成"不支持视觉"，图片被换成一句占位文字丢掉，你只会看到模型说"我看不见图"。现在 Flash 系（含旧名 `deepseek-v4-flash`）按支持处理；`deepseek-v4-pro` 仍按不支持（官方明写它不看图）。

</details>

<details><summary>使用指引里「进程管理」那一步修好了</summary>

上一版把进程按钮收进设置之后，指引里那一步的锚点失效了（不报错，但会闪一下然后跳过）。现在这一步去掉，「通用设置」那一步里写明了新位置和快捷键。
...
<details>
<summary><b>v0.2.387</b>(2026-09-14)· 修复 1 条</summary>

**修复**

<details><summary>界面字体调大后，「窗内检索」不再冲过头</summary>

搜索一条在视野外的命中、点「下一个」时，落点以前会**按界面缩放倍数偏**——字体调到「大」（1.2×）时命中被推到视野**上方**，调到「超大」（1.45×）时**整个飞出视野**（等于"搜到了但看不见"）。现在统一按布局单位计算，三种缩放下落点一致（实测 0.334 / 0.336 / 0.336，正确值是 0.333）。
...
<details>
<summary><b>v0.2.386</b>(2026-09-13)· 优化 1 条 · 修复 2 条</summary>

**优化**

<details><summary>「刷新价格」默认只刷你当前在用的那一家</summary>

以前点一次会向 17 个地址发请求（43 家里有采集器的那些）；现在默认只刷当前 provider（**1 个请求**），想看别家可以点旁边的「刷新全部 43 家」。当前 provider 没有对应的官方价目来源时，按钮会禁用并写明原因。

</details>

**修复**

<details><summary>条带摘要里不再露出 markdown 记号</summary>

摘要行是纯文本（正文是渲染过的），以前会原样显示 `**加粗**` 这样的符号。现在**成对的记号会被剥掉**（`**粗**` → `粗`、`` `代码` `` → `代码`），**零散的一个不碰**（`foo_bar_baz`、`2*3*4`、`x**2 + y**2` 都保持原样）。

</details>

<details><summary>摘要尾句不再和正文重复</summary>

以前"回合还在跑、以工具调用结尾"时，摘要会摘正文的一句话，和紧跟着的正文一字不差。现在**只用思考当尾句**；没有思考就不显示尾句。
...
<details>
<summary><b>v0.2.385</b>(2026-09-13)· 修复 2 条</summary>

**修复**

<details><summary>刷新页面后，正在跑的工作流不再丢进度</summary>

以前刷新或重开应用后，那条工作流卡片会停在「此运行未提供进度信息」——因为记录只在"工作流刚启动"那一刻创建，刷新时那一刻早就过去了，后来的进度只往**已存在**的记录上贴，没人建就什么都没有；重连回放也补不回来。现在**新的进度一到就把记录接回来**（按正确的会话归属建，且不编造启动时间，所以不会显示一个假的耗时）。工作流跑完照常定格。

</details>

<details><summary>同一个脚本续跑时，不再显示上一轮的数据</summary>

卡片缓存的键以前只有"运行 id"，而续跑会**覆写同名文件**，于是缓存永远命中最开始那一轮——连"已被取代"的提示都会写反。现在按"运行 id + 任务 id"区分，并加了上限（最多留 32 份，超了淘汰最久没用的）。
...
<details>
<summary><b>v0.2.384</b>(2026-09-13)· 更正 1 条 · 修复 1 条</summary>

**更正**

<details><summary>更正一条旧说明（0.2.379 的 computer use 那条）</summary>

那条里说「后台点击/输入只作用于你显式授权并指定的那个窗口」，还说「没有单独勾选『允许主屏全部可见内容』时，截屏会明确拒绝」——这套**按应用授权 + 主屏范围授权的界面目前还没有做出来**，当时是把设计意图当成已有功能写了。现在要用 computer use，授权只能通过接口完成：`POST /api/computer-use/grants`（`{"bundleId":"…","granted":true}` 授权某个应用，`{"screenScope":true}` 允许主屏截图），未授权的行为与那条描述一致。功能本身（后台点击不抢前台、未授权时截屏拒绝、撤销立即生效）没有变化，缺的是界面入口，特此更正。

</details>

**修复**

<details><summary>上一版给「往上滚读历史时视图被顶跑」加的补偿，在 Windows 上会帮倒忙</summary>

自带滚动锚定的浏览器（Windows 用的引擎就是）会**先自己补一遍**，我们的补偿紧接着又补一遍 → 多滚整整一段（实测漂移 300 像素）。现在改成按"变化前的基准"算目标位置，两种浏览器下都收敛（实测 0 像素）。**macOS 用户看不到区别**，这是为 Windows 修的一处。
...
<details>
<summary><b>v0.2.383</b>(2026-09-13)· 新增 2 条 · 优化 1 条</summary>

**新增**

<details><summary>一轮问答的过程收成一条条带</summary>

以前思考、工具调用、子代理这些过程块一条条竖排，一轮里调了几次工具就能占好几屏，把回复正文挤出视野。现在**整轮的过程收成一条可展开的条带**，正文直接就在眼前；想看过程点一下条带就展开，里面每一条的样子跟原来一样、各自还能单独展开。**回答出错、或被你手动中断时，条带保持展开**——不会把出问题的现场折起来。

</details>

<details><summary>后台任务的通知改成居中横幅</summary>

以前它靠左铺开、内容长的时候占掉半屏。现在跟「技能调用」那种横幅一样居中，**默认收起成一行**（仍然看得出是运行中还是已完成），要看详情点一下展开。

</details>

**优化**

<details><summary>拖窗口宽窄、开分屏不再卡了</summary>

这是本次最主要的一项。查下来卡的原因跟计算无关——是**页面上的元素太多**（一个几千条消息的会话有 2 万多个元素），每次改窗口宽度浏览器都要把所有这些元素重新算一遍排版。现在**只保留最近 30 轮在页面上**（往上滚动会自动补回更早的），实测拖窗口每格从 **194 毫秒降到 16 毫秒**。滚动条、吸底、进度条跳转这些行为**跟以前完全一样**（没有用估算高度糊弄）。
...
<details>
<summary><b>v0.2.382</b>(2026-09-13)· 修复 7 条 · 新增 1 条 · 优化 4 条</summary>

**修复**

<details><summary>内置终端不再是一块白纸</summary>

以前不管什么主题，终端都是白底黑字、跟应用整体色调打架。现在**跟着当前主题走**，切换主题时**已经开着的终端当场就变**、不用重开；`ls -G`、`git status` 这类彩色输出也做了适配，深浅两种主题下都看得清；字体跟代码块对齐。

</details>

<details><summary>打开应用、点开项目要等很久</summary>

查下来是**每次都在把会话文件的全文从头读到尾**（最大的项目一次要读几百 MB，3 GB 级的超大目录一次要读到十秒级）。现在改为只读文件头尾的摘要并缓存，**项目列表从数百毫秒降到几十毫秒、大项目的会话列表从秒级降到百毫秒级**。第一次打开一个从没缓存过的项目仍要扫一次，之后就快了。

</details>

<details><summary>排队的长消息把输入框顶飞</summary>

连续发消息时，排队条里的长消息会整段铺开（60 行的消息能撑到 650 像素高）。现在**永远只占一行**，超出用省略号截断，鼠标悬停仍能看到全文。

</details>

<details><summary>点「终端」时设置坞的按钮也跟着变成终端</summary>

那个按钮本来只该显示坞里的内容，而终端并不在坞里。现在终端开着时它保持「设置」的样子。

</details>

<details><summary>生图面板清空后，切走再切回来内容又冒出来</summary>

清空只清了眼前这份，切走时面板被卸载、回来时又自动选中了最近一张完成图。现在清空就是清空，切走切回不会复现。

</details>

<details><summary>「查看备份」点下去整个界面卡死</summary>

那个按钮会把**整份 16 MB 的会话原文**塞进页面渲染，实测卡 **38 秒**完全没响应。现在不再在应用内打开原文，改成**「在访达中显示」**，直接帮你定位到那个备份文件（响应 0.13 秒）。

</details>

<details><summary>队列里的「⚡ 并入」灰着却不说为什么</summary>

当队首那条正处于「已送进本轮、等模型读到」的状态时，它后面所有消息的并入按钮都会置灰——而那条在队列里**根本不显示**，悬停提示还写着"把队列里的下一条消息并入当前回合"（说能点、按钮是灰的）。现在那条会如实显示出来并带一个秒表，按钮灰着时提示也会说清真实原因。

</details>

**新增**

<details><summary>子代理的花费在它跑完的当下就显示</summary>

以前要等整个回合结束、界面重读一次会话记录才看得到（长回合可能十几分钟都看不到）。现在子代理一结束就立刻显示。

</details>

**优化**

<details><summary>订阅额度（官方）只在对的地方显示</summary>

它是 Claude 官方订阅（Pro/Max）的用量上限，走 API Key 的档位和第三方 provider 都用不上。现在只在官方订阅下显示；走 API Key 时保留卡片并说明情况；第三方 provider 整块不渲染（那一档看的是账户余额，是另一个接口）。**服务端只要真的取到了额度数据，一律照常显示**，不会因为身份判断失误把真额度藏起来。

</details>

<details><summary>轮末的金额不再被来源文字撑到换行</summary>

价格来源（官方价 / 分时段 / 估算…）收进金额的悬停提示，鼠标停上去才看；面板窄时不会再因为一长串来源文字折行。

</details>

<details><summary>轮末的 token 明细不再那么技术化</summary>

「实际输入」这个名字容易让人以为自己的上下文有六万多 token（它其实是**这一轮所有 API 调用的输入量之和**——一轮里每调一次工具都要重读一遍上下文），已改名为「本轮累计读取」并收进悬停；行内只留 输入 / 输出 / 花费 三项。「整轮命中率」的名字和数值**一个字没改**，只是移到了悬停里。

</details>

<details><summary>「刷新价格」的逐家结果默认折叠</summary>

以前 43 家逐条铺开会很长，现在默认收起、需要时点开。
...
<details>
<summary><b>v0.2.381</b>(2026-09-12)· 修复 1 条</summary>

**修复**

<details><summary>「并入」被误判为失败，把整条队列卡住</summary>

模型正在回答时插一句话（并入），界面有时会弹出「并入结果无法确认，已暂停后续队列」，还给出「取回为新消息 / 保留不发」两个选择。查下来这是**误判**——并入其实**已经成功写进了会话记录**，只是它落盘要等下一次工具执行完（实测等待从 3 秒到 148 秒不等），而界面的确认环节只等固定 20 秒，超时就判失败，且判定后不会自动回头。现在改成：**只要这一轮还在跑就一直等下去**，只有这一轮彻底结束后内容仍然找不到，才判失败。**真的失败时照样会拦住队列，不会漏**。
...
<details>
<summary><b>v0.2.380</b>(2026-09-12)· 修复 6 条 · 新增 3 条 · 优化 1 条</summary>

**修复**

<details><summary>DeepSeek 的花费按真实时段计算</summary>

官方 2026-08-17 起实行峰谷价（工作日 9–12 点与 14–18 点为高峰，其余时段与周末全天半价）。以前一律按一个固定价算，你填的又是高峰价，于是**夜间与周末跑的量被显示成两倍**。现在按每条记录自己的时间判高峰/空闲，逐条各算各的。

</details>

<details><summary>Claude 的缓存写入按两档分别计价</summary>

官方对"存 5 分钟"收输入价的 1.25 倍、"存 1 小时"收 2 倍。以前把 1 小时档也按 1.25 倍算，**这部分金额一直显示偏低**（你的记录里绝大多数走的是 1 小时档）。现在分开算。

</details>

<details><summary>`glm-5.3-flash` 按官方价计价</summary>

它以前被当成同族的 `glm-5` 算，输入价虚高 5 倍、输出价虚高 6 倍。现在按智谱官方价（输入 0.8 / 输出 2.8 / 缓存命中 0.23 元每百万）。

</details>

<details><summary>官方价目命不中</summary>

官方页写的模型名与本地 id 常有大小写差异（如 `GLM-5.3-Flash` 对 `glm-5.3-flash`），逐字比较永远对不上，会静默退回旧价。现在按统一口径比对，能命中官方价。

</details>

<details><summary>只填了部分单价时不再猜</summary>

手填单价只填了输入/输出、没填缓存价时，以前会用"输入价 × 0.1 / × 1.25"补一个数出来。现在未填的项按"未知"处理、如实标注，不显示一个猜出来的金额。

</details>

<details><summary>`gpt-5.6` 系列价格更新</summary>

，并对超过 272K 输入的长请求按官方的长上下文档计费。

</details>

**新增**

<details><summary>子代理的花费显示在它自己的位置上</summary>

以前子代理花了多少完全看不到。现在前台子代理在对话卡片和监控面板两处都显示；后台运行的子代理只在监控面板显示；工作流（workflow）卡片显示它内部所有子代理的合计、面板里逐条列出。按每个子代理自己用的模型与调用时间计价。读不到用量或算不出价的，显示「未能计价」而不是显示 0。

</details>

<details><summary>填的 baseURL 撞上官方预设时会提示你</summary>

如果地址正好是某个官方入口（换哪种协议都算），保存时会问你要不要切到那个预设——走预设才能自动带上正确的计价规则与额度接口。不点确认不会改动你的配置。

</details>

<details><summary>OpenAI 的额度改走本机 codex 通道</summary>

额度面板能读 OpenAI 的余量了（需本机装有 codex）。

</details>

**优化**

<details><summary>读不到额度的 provider 如实说明</summary>

区分"这家本来就没有额度接口"与"有接口但本期还没接入"，都提示去官网查看，不显示假数字。
...
<details>
<summary><b>v0.2.379</b>(2026-09-11)· 修复 7 条 · 新增 1 条 · 优化 2 条</summary>

**修复**

<details><summary>切会话/切窗格时另一个会话的内容串进来（用户实报"串台"）</summary>

现在每个窗格有自己的身份（会话/draft + 本轮代际 + 运行 id），一个会话迟到的正文、模型、运行状态再也不会写进你正在看的另一个会话；关掉一个窗格只是停止显示，它后台的 CLI 照常跑完并把结果落进历史；切回来能看到自己的后台运行，查不到就明说"暂无可恢复运行"，不会拿别的会话顶替。

</details>

<details><summary>断线恢复后刚发出的消息会消失</summary>

真断线（不是切走）恢复时，那条消息原先会从列表里消失十几秒（要等回合结束才回来）。现在按会话记录就地换成真实 id，全程都在、不会变成两条。

</details>

<details><summary>轮末的花费与「整轮命中率」只算了最后一次调用</summary>

一轮里模型多次调用（带工具、带思考）时，以前显示的是**最后一次**那笔的用量与花费，于是钱看着明显偏少、命中率看着明显偏低（尤其第三方 provider）。现在改为整轮累计口径。

</details>

<details><summary>任务清单点「隐藏」之后会自己弹回来</summary>

列表内容一有更新（勾选、加任务）卡片就重新出现。现在隐藏就是隐藏，只有你自己点「显示任务清单」才回来；隐藏状态按会话记，换会话不受影响。

</details>

<details><summary>并入的消息</summary>

附件以前显示成一串文件路径，现在和普通消息一样出缩略图/文件卡并可以点开；并入的消息也能回退了（以前只对普通消息有效）。

</details>

<details><summary>侧栏项目列表会冒出临时目录</summary>

会话里跑过的临时/内部工作目录（`.scratch/…`、`.claude/worktrees/…` 这类）会像你自己的项目一样列出来。现在这类目录按 worktree 看待、默认不显示（打开「显示 worktree」可见），你自己的项目一个不受影响。

</details>

<details><summary>历史操作（裁剪 / 去掉思考 / 压缩）会直接改会话</summary>

现在先给预览（不写任何东西），确认后才提交，并且可以查看这次操作的备份；用不上的操作会被明确拒绝，不再悄悄改写。

</details>

**新增**

<details><summary>官方价格与用量展示</summary>

价格区从 43 个预设各自的官方来源取价（分时/分档、多币种、缓存读写分别列），能展开看到来源链接、抓取时间、适用条件与过期/估算/缺项；「刷新价格」逐家显示成功/部分成功/失败，取不到的源如实标未映射或来源不可用，不会编一个价出来。用量上，"最近 API 命中率 / 整轮命中率 / 会话累计命中率"三处各自标注口径，分母为 0 显示「—」。

</details>

**优化**

<details><summary>电脑操控（computer use）不再误伤你的桌面</summary>

后台点击/输入只作用于你显式授权并指定的那个窗口，失败时不会"退而求其次"去点你前台的应用；没有单独勾选"允许主屏全部可见内容"时，截屏会明确拒绝而不是把整块屏幕交出去；后台的 `cmd+a` 也能真正生效了。

</details>

<details><summary>官方额度查询</summary>

额度卡片在非官方通道下不再装作有数据，写明"额度暂不可用"并给出在官方 CLI 查看的入口；查询不再自读订阅凭证、不冒充 CLI。
...
<details>
<summary><b>v0.2.378</b>(2026-09-07)· 优化 1 条</summary>

**优化**

- 设置 → 通用 里的按钮文字由「查看更新说明」改为「更新说明」。
