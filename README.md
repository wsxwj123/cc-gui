...
以前点一次会向 17 个地址发请求（45 家里有采集器的那些）；现在默认只刷当前 provider（**1 个请求**），想看别家可以点旁边的「刷新全部 45 家」。当前 provider 没有对应的官方价目来源时，按钮会禁用并写明原因。
...
价格区从 45 个预设各自的官方来源取价（分时/分档、多币种、缓存读写分别列），能展开看到来源链接、抓取时间、适用条件与过期/估算/缺项；「刷新价格」逐家显示成功/部分成功/失败，取不到的源如实标未映射或来源不可用，不会编一个价出来。用量上，"最近 API 命中率 / 整轮命中率 / 会话累计命中率"三处各自标注口径，分母为 0 显示「—」。
