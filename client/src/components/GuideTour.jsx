...
function buildSteps(hasProject) {
  ...
  // ── ③ 左栏(r11-①:两页切换 —— 项目页 ⇄ 会话页)────────────────────────
  steps.push(
    ['sidebar-list', '项目与会话', '两页切换:项目页只列项目名,点某个项目进入其会话页(顶部「返回项目列表」可回来),点某条会话才进入它。\n· 项目行 hover:在文件夹中显示、pin 置顶、隐藏;进入项目后操作条含「新建 / worktree」、切「活跃 / 已归档」、彻底清理\n· 会话可置顶、归档、删除、分叉,点标题重命名\n· 顶部搜索框同时搜项目路径、会话标题与消息全文'],
    ['add-project', '添加项目文件夹', '把一个本地文件夹加进来作为新项目。\n· 点这里弹系统文件夹选择器,或手动粘贴绝对路径'],
  );
  if (hasProject) {
    steps.push(
      // 锚点在会话页操作条:未钻入项目时元素不在 DOM,本步自动跳过(findEl 宽度守卫)。
      ['new-session', '新建会话', '进入某个项目后,在操作条开一个新会话(也可按 Cmd/Ctrl+N)。\n· 自动继承上一个会话的推理力度,免得每次重选\n· 旁边的「worktree」按钮在隔离的 git worktree 里开会话,改动不污染当前分支'],
    );
  }
  // ── ④ 输入框(底部,从左到右)+ ⑤ help 收尾 ────────
