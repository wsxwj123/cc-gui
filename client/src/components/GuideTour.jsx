...
function buildSteps(hasProject) {
  ...
  // ── ③ 左栏(r10-11 单面板:项目折叠列表,两套文案合并)──────────────────
  steps.push(
    ['sidebar-list', '项目与会话', '所有项目文件夹的折叠列表:点项目行展开其会话(不切走当前会话),点某条会话才进入它。\n· 项目行可 pin 置顶、隐藏、彻底清理该项目的 Claude 状态\n· 展开后:组内「新建 / worktree」按钮、切「活跃 / 已归档」;会话可置顶、归档、删除、分叉,点标题重命名\n· 顶部搜索框同时搜项目路径、会话标题与消息全文'],
    ['add-project', '添加项目文件夹', '把一个本地文件夹加进来作为新项目。\n· 点这里弹系统文件夹选择器,或手动粘贴绝对路径'],
  );
  if (hasProject) {
    steps.push(
      ['new-session', '新建会话', '在当前项目组头开一个新会话(也可按 Cmd/Ctrl+N)。\n· 自动继承上一个会话的推理力度,免得每次重选\n· 旁边的「worktree」按钮在隔离的 git worktree 里开会话,改动不污染当前分支'],
    );
  }
  // ── ④ 输入框(底部,从左到右)+ ⑤ help 收尾 ────────
