...
function buildSteps(hasProject) {
  // 顺序 = 视觉从左到右、从上到下(顶栏→左栏→中区会话头→底部输入框):①顶栏(最左侧栏开关 → 右簇 Provider/模型/力度/
  // 远程/主题/坞 → 坞内分屏+面板)②左栏(列表 / 新建)③会话头(徽章 / ⋮)④输入框
  ...
  const steps = [
    ...
    ['model-selector', '模型', '选当前会话使用的具体模型(分屏时作用于聚焦的窗格,每格独立)。\n· 支持 1M 上下文开关、搜索、手填自定义模型 ID\n· 「拉取最新」打开勾选弹窗:勾选的模型才进列表,重拉不重置已勾选(自定义 provider)'],
    ['effort-selector', '推理力度', '调 AI 的思考强度:低 / 中 / 高 / 极高 / 极限(分屏时作用于聚焦的窗格)。\n· 越高思考越深入、结果越细致,但越慢、越费 token\n· 官方模型区别明显,部分第三方可能无效'],
    ['remote-control', '远程控制', '用手机 Claude App 同账号接管当前会话(需先发送一条消息创建会话)。\n· 需官方 Anthropic 端点 + claude.ai 订阅登录;切到第三方 provider 时该按钮置灰\n· 激活后输入框锁定,再点一次收回控制'],
    ['theme-toggle', '主题与外观', '外观相关设置的唯一入口:\n· 明暗 / 配色主题(多套深浅色)、界面字号、对话正文字体\n· AI 思考时的加载动画样式(30 种可选)\n· 聊天模式(折叠思考 / 工具只看对话文本)、对话区背景(纯色 / 图片 / 视频)\n· 皮肤:内置 Windows XP / 初音未来 / QQ2008 / 鲸歌等,支持导入(zip / 文件夹)与「AI 生成」提示词生成皮肤包'],
    ['panel-dock', '设置(面板坞)', '分屏和所有功能面板都收纳在这:点它原位展开面板条,再点任一图标打开对应面板;展开后常驻,再点坞图标才收起。\n· 快捷键 Cmd/Ctrl+1..9 直达前 9 个面板、0 打开「通用」(面板条收起时同样可用)\n· 有可用更新时图标出现红点,展开后点「更新」直达更新区', 'cgui:dock-rail-open'],
    ['dock-pane', '分屏', '把界面分成 1–6 个窗格,并排同时看和操作多个会话。\n· 每个窗格的模型 / 权限模式 / 力度相互独立\n· Ctrl+Tab 轮换聚焦窗格', 'cgui:dock-rail-open'],
    ...PANEL_STEPS,
  ];
  // ── ③ 左栏(r13-①:折叠树 —— 项目/会话二合一)────────────────────────
  steps.push(
    ['sidebar-list', '项目与会话', '折叠树:项目行左侧是折叠符,点一下展开该项目的会话,再点收起;点某条会话才进入它(展开≠选中)。\n· 项目行 hover 出两枚按钮:「+」在该项目下新建会话,「⋯」收纳在文件夹中显示 / 置顶 / 隐藏 / 新建 worktree / 查看已归档\n· 会话行 hover 的「⋯」含置顶、重命名、分支会话、归档、删除\n· 顶部搜索框同时搜项目路径、会话标题与消息全文'],
    ['sidebar-view', '分组与排序', '侧栏头部这枚按钮控制列表的组织方式。\n· 分组:按项目(折叠树)或单列表(所有会话跨项目按时间排,不显示项目行)\n· 排序:最近更新,或手动排序(桌面端拖拽项目行重排;置顶的项目恒在最前)\n· 选择存在服务端,手机与电脑共享同一套'],
    ['add-project', '添加项目文件夹', '把一个本地文件夹加进来作为新项目。\n· 点这里弹系统文件夹选择器,或手动粘贴绝对路径'],
  );
  if (hasProject) {
    steps.push(
      // 锚点在项目行 hover 出的「+」:项目行未渲染时元素不在 DOM,本步自动跳过(findEl 宽度守卫)。
      ['new-session', '新建会话', '点项目行的「+」(或按 Cmd/Ctrl+N)开新会话,主区会显示新建会话页:问候语、输入框与项目选择器。\n· 在那里先选项目再发第一条消息,会话才真正建立\n· 自动继承上一个会话的推理力度,免得每次重选\n· 「⋯」菜单里的「新建 worktree 会话」在隔离的 git worktree 里开会话,改动不污染当前分支'],
    );
  }
  // ── ③ 会话头(中区顶部;r61b:左栏在前,遵循从左到右、从上到下)──
  steps.push(
