...
export default function TurnScrubber({ containerRef, turns, btwTurnUuids }) {
  ...
  return (
    ...
    >
      {positions.map((n, i) => {
        ...
        const hasBtw = btwTurnUuids?.has(t.uuid);
        return (
        <button
          key={t.uuid || i}
          ...
          // 含旁问的回合:点用 accent 实色 + 外环,与普通回合区分(悬浮 tooltip 另有说明)
          className={hasBtw
            ? 'w-[6px] h-[6px] rounded-full bg-accent ring-2 ring-accent/30 cursor-pointer'
            : 'w-[6px] h-[6px] rounded-full bg-ink-faint/50 hover:bg-accent cursor-pointer'}
          aria-label={`跳到第 ${i + 1} 个回合${hasBtw ? '(含旁问)' : ''}`}
        />
        );
      })}
      {tipIdx != null && positions[tipIdx] != null && turns[tipIdx] && (
        ...
        >
          <div className="text-[10px] text-ink-faint font-mono mb-0.5 flex items-center gap-1.5">
            ...
            {btwTurnUuids?.has(turns[tipIdx].uuid) && (
              <span className="text-accent font-body">· 含旁问</span>
            )}
          </div>
