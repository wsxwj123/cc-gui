...
import { claimAttach, releaseAttach } from '../../server/routes/chat.js';
...
const mkConn = (name, log) => {
  const onLine = (l) => log.push(`${name}:${l}`);
  const conn = { onLine, ended: 0, end: () => { conn.ended++; } };
  return conn;
};
...
// ── ② 已 attach:接管 —— 老响应被 end、监听清空、attached 仍真、token 更新 ──
{
  ...
  assert.deepEqual(log, ['new:hello'], '接管后只有新连接收到行');
}
...
// ── ⑥ 老响应 end() 自身抛错(socket 已死)不得中断接管 ──────────────────
{
  const slot = mkSlot();
  slot.attached = true;
  slot.attachToken = 1;
  const boom = { onLine: () => {}, end: () => { throw new Error('EPIPE'); } };
  slot.listeners.add(boom.onLine);
  slot.attachments.add(boom);
  assert.doesNotThrow(() => claimAttach(slot, 2));
  assert.equal(slot.attached, true);
  assert.equal(slot.attachToken, 2, 'end 抛错也必须走完接管,否则新窗格接不上');
  assert.equal(slot.listeners.size, 0);
}
