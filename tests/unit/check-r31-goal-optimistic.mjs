...
import { parseGoalCommand } from '../../client/src/utils/goal.js';
...
assert.ok(app.includes('setOptimisticGoal'), 'App 要能写乐观态(setter)');
...
assert.ok(/setOptimisticGoal\([\s\S]*?\? null : \{ met: false, sentinel: true, condition: _gc.condition \}\)/.test(app)
  || /setOptimisticGoal\(\s*_gc\.type === 'clear'\s*\? null : \{ met: false, sentinel: true, condition: _gc.condition \}?/.test(app),
  '/goal clear → null,其余 set + met:false/sentinel:true');
...
const sendSetIdx = app.indexOf('if (_gc) setOptimisticGoal(');
