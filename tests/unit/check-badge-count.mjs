...
// ── 3. 防抖:计数没变就不下发 ────────────────────────────────────────────
{
  ...
  assert.deepEqual(first, { count: undefined, title: 'Claude GUI' }, '首次(0)要下发一次,把可能的残留角标清掉');
  assert.equal(nextBadgeState(0), null, '同一计数不重复下发(1.5s 轮询下多数轮次都不变)');
  assert.deepEqual(nextBadgeState(2), { count: 2, title: 'Claude GUI (2)' }, '变了才下发');
  assert.equal(nextBadgeState(2), null);
  assert.deepEqual(nextBadgeState(0), { count: undefined, title: 'Claude GUI' },
