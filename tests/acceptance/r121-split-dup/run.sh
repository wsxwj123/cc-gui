#!/usr/bin/env bash
# r121 界面验收(分屏状态下一条消息被渲染两次),一条命令:
#   tests/acceptance/r121-split-dup/run.sh                 # 全量
#   tests/acceptance/r121-split-dup/run.sh -g 'B1'         # 额外参数原样转给 playwright
#   R121_PROBE=1 tests/acceptance/r121-split-dup/run.sh -g '探路'   # 只跑探路脚本
# 自己做:建夹具(隔离 HOME + 两条会话)→ 起隔离实例(HOME=本套件 .artifacts,PATH 上挂假 claude)
...
echo "[r121] 隔离实例就绪:http://127.0.0.1:$API_PORT(pid $API_PID,HOME=$HOME_DIR)"

( cd "$WORKTREE/client" && R121_API_PORT="$API_PORT" R121_UI_PORT="$UI_PORT" \
...
if [ -n "$LISTENER" ] && ps -p "$LISTENER" -o command= | grep -q "r121-split-dup"; then
  VITE_PID="$LISTENER"
else
  echo "dev server 没起来或端口 $UI_PORT 上不是本套件的进程:看 $LOGS/vite.log" >&2; exit 1
fi
echo "[r121] dev server 就绪:http://127.0.0.1:$UI_PORT(pid $VITE_PID;/api、/ws 代理到 $API_PORT)"

cd "$WORKTREE"
set +e
R121_UI_BASE="http://127.0.0.1:$UI_PORT" R121_API_BASE="http://127.0.0.1:$API_PORT" R121_CTL="$CTL" \
  npx playwright test -c "$SUITE/playwright.config.mjs" "$@"
CODE=$?
set -e
echo "[r121] playwright 退出码 $CODE(实例 pid $API_PID / dev server pid $VITE_PID)"
exit $CODE
