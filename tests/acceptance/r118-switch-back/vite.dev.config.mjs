// 页面来源 = dev server(源码直出),不是 client/dist。/api、/ws 全部代理到本套件的隔离实例。
// cacheDir 指到本套件 .artifacts:不写 client/node_modules/.vite,不碰别人的缓存。
import { fileURLToPath } from 'node:url';
import base from '../../../client/vite.config.js';

const apiPort = Number(process.env.R118_API_PORT || 0);
const uiPort = Number(process.env.R118_UI_PORT || 0);
if (!apiPort || !uiPort) throw new Error('R118_API_PORT / R118_UI_PORT 必须先设置(run.sh 负责)');
