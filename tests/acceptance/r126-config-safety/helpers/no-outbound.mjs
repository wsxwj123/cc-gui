// r126 · 隔离实例的"断外网"预载(NODE_OPTIONS=--import=<本文件的 file:// URL>);照 r125 的写法。
// 目的:验收规则"不联外网"。产品若去打任何非回环地址,这里让它拿到一个普通的网络错误(ENETUNREACH),
// 并把被拒的目标记到 R126_BLOCKED_LOG,供用例取证。
// 只拦 TCP 客户端连接(net.Socket#connect 是 http/https/undici/ws 出站的共同入口),不碰监听。
// 放行规则:只放 127.0.0.1/localhost 且端口在 R126_ALLOW_PORTS(默认 6700-6999)之内 ——
...
const [lo, hi] = (process.env.R126_ALLOW_PORTS || '6700-6999').split('-').map(Number);
const portAllowed = (p) => Number(p) >= lo && Number(p) <= hi;
const logFile = process.env.R126_BLOCKED_LOG;
...
net.Socket.prototype.connect = function r126GuardedConnect(...args) {
  const t = target(args);
  if (t && !(isLoopback(t.host) && portAllowed(t.port))) {
    const err = Object.assign(new Error(`r126 no-outbound: refused ${t.host}:${t.port}`), { code: 'ENETUNREACH', errno: -51, syscall: 'connect', address: t.host, port: t.port });
