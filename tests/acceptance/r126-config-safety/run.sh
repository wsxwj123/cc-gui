#!/usr/bin/env bash
# r126 「GUI 配置文件损坏时不许被覆盖写、先备份、警告可见」验收,一条命令:
#   tests/acceptance/r126-config-safety/run.sh                  # 全量(B / C 两组)
#   tests/acceptance/r126-config-safety/run.sh -g 'C1'          # 额外参数原样转给 playwright
#   R126_API_ONLY=1 tests/acceptance/r126-config-safety/run.sh  # 不起 dev server,只跑接口用例(界面用例自动跳过)
# 自己做:建共享实例的隔离 HOME → 起共享隔离实例(HOME=本套件 .artifacts,挂断外网预载)→ [起 dev server(源码直出,
#        /api、/ws 代理到共享实例)] → 跑用例(接口用例各自再起自己的实例)→ 只按记录的 pid 收尾。
...
env -u ANTHROPIC_BASE_URL -u ANTHROPIC_AUTH_TOKEN -u ANTHROPIC_MODEL -u ANTHROPIC_API_KEY -u CLAUDE_CODE_OAUTH_TOKEN \
  ...
  NODE_OPTIONS="--import=$NO_OUTBOUND_URL" R126_BLOCKED_LOG="$LOGS/shared.blocked.log" \
...
echo "[r126] 共享隔离实例就绪:http://127.0.0.1:$API_PORT(pid $API_PID,HOME=$HOME_DIR)"

# 2) dev server
UI_BASE=""
if [ "${R126_API_ONLY:-0}" != "1" ]; then
  ( cd "$WORKTREE/client" && R126_API_PORT="$API_PORT" R126_UI_PORT="$UI_PORT" \
  ...
  if [ -n "$LISTENER" ] && ps -p "$LISTENER" -o command= | grep -q "r126-config-safety"; then
    VITE_PID="$LISTENER"
  else
    echo "dev server 没起来或端口 $UI_PORT 上不是本套件的进程:看 $LOGS/vite.log" >&2; exit 1
  fi
  UI_BASE="http://127.0.0.1:$UI_PORT"
  echo "[r126] dev server 就绪:$UI_BASE(pid $VITE_PID;/api、/ws 代理到 $API_PORT)"
else
  echo "[r126] R126_API_ONLY=1:不起 dev server,界面用例将跳过"
fi

# 3) 跑用例
cd "$WORKTREE"
set +e
R126_UI_BASE="$UI_BASE" R126_API_BASE="http://127.0.0.1:$API_PORT" R126_HOME="$HOME_DIR" R126_LOGS="$LOGS" \
  npx playwright test -c "$SUITE/playwright.config.mjs" "$@"
CODE=$?
set -e
echo "[r126] playwright 退出码 $CODE(实例 pid $API_PID / dev server pid ${VITE_PID:-无};产物在 $ROOT)"
exit $CODE
