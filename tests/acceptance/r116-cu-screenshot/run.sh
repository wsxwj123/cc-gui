#!/usr/bin/env bash
# r116 界面验收,一条命令:
#   tests/acceptance/r116-cu-screenshot/run.sh               # 全量
#   tests/acceptance/r116-cu-screenshot/run.sh -g 'UI-H'     # 额外参数原样转给 playwright
# 自己做:建夹具 → 起隔离实例(HOME=本套件 .artifacts,PATH 上挂假 claude)→ 起 dev server(源码直出)
#        → 跑用例 → 按记录的 pid 收掉这两个进程。
# 端口只在 6860–6999 里挑空闲的,硬拒 6677 / 6689 / 6710。
set -euo pipefail

SUITE="$(cd "$(dirname "$0")" && pwd)"
WORKTREE="$(cd "$SUITE/../../.." && pwd)"
# 每次一个全新的数据根(HOME 在它下面):上一轮的会话、索引缓存不会混进来。旧目录不自动删,都在已忽略的 .artifacts 里。
ROOT="$SUITE/.artifacts/runs/$(date +%Y%m%d-%H%M%S)-$$"
LOGS="$ROOT/logs"
mkdir -p "$LOGS"
export R116_DATA_ROOT="$ROOT"

# 用 lsof 看有没有监听者(对 127.0.0.1 试绑在 macOS 上会被 0.0.0.0 的监听骗过)
pick_port() {
  local exclude=" $* " port
  for port in $(seq 6860 6999); do
  ...
  echo "6860-6999 没有空闲端口" >&2; return 1
}
API_PORT="$(pick_port)"
...
rm -f "$CTL/release"   # 上一轮留下的放行文件(单个文件)
...
echo "[r116] 隔离实例就绪:http://127.0.0.1:$API_PORT(pid $API_PID,HOME=$ROOT/home)"

( cd "$WORKTREE/client" && R116_API_PORT="$API_PORT" R116_UI_PORT="$UI_PORT" \
...
if [ -n "$LISTENER" ] && ps -p "$LISTENER" -o command= | grep -q "r116-cu-screenshot"; then
  VITE_PID="$LISTENER"
else
  echo "dev server 没起来或端口 $UI_PORT 上不是本套件的进程:看 $LOGS/vite.log" >&2; exit 1
fi
echo "[r116] dev server 就绪:http://127.0.0.1:$UI_PORT(pid $VITE_PID;/api、/ws 代理到 $API_PORT)"

cd "$WORKTREE"
set +e
R116_UI_BASE="http://127.0.0.1:$UI_PORT" R116_API_BASE="http://127.0.0.1:$API_PORT" R116_CTL="$CTL" \
  npx playwright test -c "$SUITE/playwright.config.mjs" "$@"
CODE=$?
set -e
echo "[r116] playwright 退出码 $CODE(实例 pid $API_PID / dev server pid $VITE_PID)"
exit $CODE
