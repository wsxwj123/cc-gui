...
本清单锁定 22 条独立黑盒验收结果，目标是修复“GUI 默认插件全部安装失败”，并防止超时、代理、缓存、错误映射、秘密泄露和第三方 payload/stale 回归。测试不 import 产品内部模块，只观察真实构建 GUI、公开 HTTP、Claude CLI 2.1.240 和隔离文件系统结果。
...
- 插件管理的固定公开 HTTP 模板为：`GET /api/plugins/available`、`POST /api/plugins/install`、`POST /api/plugins/{pluginId}/update`、`DELETE /api/plugins/{pluginId}`、`PUT /api/plugins/{pluginId}/enable`、`PUT /api/plugins/{pluginId}/disable`。install 继续锁定结构化错误；其余旧路由可返回安全的 string 或 object 错误，但不得吞掉可理解的失败信息。
...
- 安全与重试分类用例允许 CLI 外壳输出固定虚假 canary stderr 并非零退出，但绝不注入成功结果。公开响应中任何字符串字段都不得出现 canary 原文；单字段最多 4096 字符，完整响应体最多 16384 字符。覆盖 auth、带单双引号 Bearer、Basic、带空格 password、嵌套 password/credential/accessToken/apiKey、ANSI CSI/OSC/C1 干扰、proxyAuthorization/clientCredential/credentials 派生键及超长文本。
...
| PLUG-SEC-ROUTES-001 | 离线 | 六个真实公开插件路由的 CLI 命令分别非零退出，并注入 auth/Bearer/Basic/password/嵌套敏感键及超长 stderr | 六个响应均非成功且非 404；递归所有字符串无 canary，敏感值消失或被遮，保留安全上下文；install 结构化，旧路由兼容安全 string/object；完整响应有界 | 诊断红 |
| PLUG-SEC-ANSI-001 | 离线 | 六个真实公开路由注入 ANSI 着色 password、CSI/OSC/C1 变体、proxyAuthorization/clientCredential/credentials 及同名递归对象、超长 stderr | 至少 install/update 且实际六路由均非成功；递归所有字段值无 canary，保留可理解安全上下文，单字段与完整响应均有界 | 诊断红 |
...
| PLUG-ERR-TERMINAL-001 | 离线 | 同一真实 stale→update 链依次返回 permission/EACCES/EPERM、401/403、invalid argument、plugin/marketplace not found，并分别混入 ECONNREFUSED/timeout/DNS/socket hang up；最后返回纯瞬态网络错误 | 五类混合终止错误均 `retryable=false`；纯瞬态网络错误 `retryable=true`；全部保留 `marketplace-update / CLI_EXIT_NONZERO / 120000` | 诊断红 |
| PLUG-ERR-NAMED-TERMINAL-001 | 离线 | stale→update 依次返回带引号的 plugin/marketplace not found、invalid marketplace name、unknown option，并分别混入 connection reset/network timeout/socket hang up/ECONNRESET；另跑纯网络反例 | 四类带名称或选项的终止错误均 `retryable=false`；纯网络仍 `true`；全部保持结构化 update 错误 | 诊断红 |
...
六类插件路由若由外部宿主改写，只允许通过以下固定模板覆盖，`{pluginId}` 由测试按 URL 规则编码：
...
- 缺宿主时，22 条应全部 skip，进程退出码仍为 0；任何语法、fixture 或探针未捕获错误必须令进程非 0。
...
运行 18 条无外网用例：设置基础宿主和本地 payload 后执行同一 `node --test` 命令；三条网络和一条长时用例会明确 skip。
...
完成时必须有 22 条独立结果、无真实配置或凭证访问、无遗留后端/GUI/CLI 子进程；网络/长时未跑必须保留明确 TEST_INFRA 标记。
...
## HEAD a832e4e 第二轮复审门禁证据

- `PLUG-SEC-ROUTES-001`：红，约 6.5 秒。六个公开路由及各自 CLI 命令均已实际命中；install 的结构化断言已通过，随后 update 响应仍泄露第一枚固定虚假 route canary。测试报告只显示 canary 编号，不回显原文。
- `PLUG-ERR-TERMINAL-001`：红，约 8.4 秒。纯瞬态反例和五种混合 stale→update 失败均已实际执行；纯瞬态保持 `retryable=true`，首个同时含 permission/EACCES/EPERM 与瞬态提示的结果也被错误标为 `true`，终止错误未取得优先级。

## HEAD 38866cf 第三轮复审门禁证据

- `PLUG-SEC-ANSI-001`：红，约 4.9 秒。六个公开路由及各自 CLI 命令均已实际命中；install 既有结构化检查先通过，随后 update 响应仍泄露第一枚 ANSI/派生键虚假 canary。测试报告只显示编号，不回显原文或控制序列。
- `PLUG-ERR-NAMED-TERMINAL-001`：红，约 7.1 秒。纯网络反例和四种命名终止组合均已执行；纯网络保持 `retryable=true`，首个带引号 plugin-not-found + connection-reset 组合也错误返回 `true`。
