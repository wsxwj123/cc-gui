...
let cache = null; // { at, providerId, data }
...
router.get('/provider-quota', async (_req, res) => {
  ...
  const now = Date.now();
  if (cache && cache.providerId === provider.id && now - cache.at < CACHE_MS) return res.json(cache.data);
  // 失败冷却:全失败后 CACHE_MS 内不再打真接口("标记无额度接口,本次不再重试")。
  // 有旧数据回放 + degraded,没有就回上次原因。
  if (cooldown.providerId === provider.id && now < cooldown.until) {
  ...
  if (!candidates.length) {
    cooldown = { providerId: provider.id, until: Date.now() + CACHE_MS, reason: 'no-endpoint', note: reasonNote('no-endpoint') };
