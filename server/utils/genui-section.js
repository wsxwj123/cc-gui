...
- LOCAL-FIRST + action：界面自己能完成的状态变化（排序、折叠、切页、选答案、判卷、重置）一律本地即时完成，零往返；\`action\` 只用于必须你参与的事。交互组件带 \`"action":"名字"\`（形态限 \`^[A-Za-z0-9_.:-]{1,64}$\`，写错形态整个组件节点被丢弃）；用户操作会以一条 \`[genui-action]\` 开头的普通用户消息把动作名与组件数据回传给你，届时重新渲染界面即可更新。要点选即回传就得写 \`action\`：不带 \`action\` 的按钮渲染成禁用态、用户点不了，不带 \`action\` 的 select / checkbox / switch 只是本地状态、不外发；让用户选一项好接着做，就给 radio 写 \`action\`、别带 \`group\`——\`group\` 只用于卷子：radio 带 \`group\` 进入聚合模式，选择只本地记录、不发往返，只有 \`submit\` 能把选中值带给你（换成 button 带不出选中值）。
